...
import Pyro4
from Pyro5.core import type_meta
...
# query for various metadata via yplookup (Yellow-Pages type lookup)
print("\nall storage:")
devices = ns.yplookup(meta_all={"resource:storage"})
...
devices = ns.yplookup(meta_any={"resource:storage", "resource:communication"})
for name, uri in devices.items():
    print("   {} -> {}".format(name, uri))
...
nameserver = Pyro4.Proxy("PYROMETA:class:Pyro5.nameserver.NameServer")
