...
class GameEngine(object):
    ...
    def start_round(self):
        ...
        # make sure the observers of all robots are owned by this thread (if they're a Pyro proxy)
        for robot in self.world.robots:
            if isinstance(robot.observer, Pyro5.api.Proxy):
                robot.observer._pyroClaimOwnership()
    ...
    def signup_robot(self, name, observer=None):
        ...
        while True:
            ...
            if not self.world.collides(x, y):
                break
        robot = VisibleRobot(name, (x, y), (0, 0), self.grid, color=color)
        self.world.add_robot(robot)
        robot.observer = observer
        robot.popuptext(name)
        return remote.RemoteBot(robot, self)
...
Pyro5.api.SerializerBase.register_class_to_dict(VisibleRobot, robot.Robot.robot_to_dict)
