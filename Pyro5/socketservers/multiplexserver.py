...
try:
    ...
    import selectors
from .. import errors, config, socketutil
...
class SocketServer_Multiplex(object):
    ...
    def init(self, daemon, host, port, unixsocket=None):
        ...
        self.sock = socketutil.createSocket(bind=bind_location, reuseaddr=config.SOCK_REUSE, timeout=config.COMMTIMEOUT,
                                            noinherit=True, nodelay=config.SOCK_NODELAY)
        self.daemon = daemon
