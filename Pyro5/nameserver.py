...
__all__ = ["resolve", "locateNS", "startNS", "startNSloop", "type_meta"]

log = logging.getLogger("Pyro5.nameserver")


class MemoryStorage(dict, MutableMapping):
    """
    ...
    def __setitem__(self, key, value):
        uri, metadata = value
        super().__setitem__(key, (uri, metadata or frozenset()))
...
class SqlStorage(MutableMapping):
    ...
    def __getitem__(self, item):
        try:
            ...
            raise errors.NamingError("sqlite error in getitem: "+str(e))
    ...
    def __setitem__(self, key, value):
        ...
        try:
            ...
            raise errors.NamingError("sqlite error in setitem: "+str(e))
    ...
    def __len__(self):
        try:
            ...
            raise errors.NamingError("sqlite error in len: "+str(e))
    ...
    def __contains__(self, item):
        try:
            ...
            raise errors.NamingError("sqlite error in contains: "+str(e))
    ...
    def __delitem__(self, key):
        try:
            ...
            raise errors.NamingError("sqlite error in delitem: "+str(e))
    ...
    def __iter__(self):
        try:
            ...
            raise errors.NamingError("sqlite error in iter: "+str(e))
    ...
    def clear(self):
        try:
            ...
            raise errors.NamingError("sqlite error in clear: "+str(e))
    ...
    def optimized_prefix_list(self, prefix, return_metadata=False):
        try:
            ...
            raise errors.NamingError("sqlite error in optimized_prefix_list: "+str(e))
    ...
    def optimized_metadata_search(self, metadata_all=None, metadata_any=None, return_metadata=False):
        try:
            ...
            raise errors.NamingError("sqlite error in optimized_metadata_search: "+str(e))
    ...
    def remove_items(self, items):
        try:
            ...
            raise errors.NamingError("sqlite error in remove_items: "+str(e))
    ...
    def everything(self, return_metadata=False):
        try:
            ...
            raise errors.NamingError("sqlite error in everything: "+str(e))
...
class NameServer(object):
    ...
    def lookup(self, name, return_metadata=False):
        ...
        try:
            ...
            raise errors.NamingError("unknown name: " + name)
    ...
    def register(self, name, uri, safe=False, metadata=None):
        ...
        with self.lock:
            if safe and name in self.storage:
                raise errors.NamingError("name already registered: " + name)
            if metadata:
...
class NameServerDaemon(server.Daemon):
    ...
    def handleRequest(self, conn):
        try:
            return super().handleRequest(conn)
        except errors.ProtocolError as x:
...
def main(args=None):
    ...
    parser.add_argument("-s", "--storage", help="Storage system to use ('memory' or 'sql:filename')", default="memory")
    parser.add_argument("--bchost", dest="bchost", help="hostname to bind broadcast server on (default is \"\")")
    parser.add_argument("--bcport", dest="bcport", type=int,
    ...
    startNSloop(args.host, args.port, enableBroadcast=args.enablebc,
                bchost=args.bchost, bcport=args.bcport, unixsocket=args.unixsocket,
                nathost=args.nathost, natport=args.natport, storage=args.storage)
